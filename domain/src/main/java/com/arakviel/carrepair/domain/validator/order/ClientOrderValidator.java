...
class ClientOrderValidator extends OrderValidator {

    ClientOrderValidator(Map<String, List<String>> validationMessages) {
    ...
    public boolean validate(Order order) {
        ...
        List<String> messages = DomainRequireValidator.getInstance()
                .getErrorMessages(order.getClient(), order.getClient().getId());

        if (!messages.isEmpty()) {
            validationMessages.put("client", messages);
