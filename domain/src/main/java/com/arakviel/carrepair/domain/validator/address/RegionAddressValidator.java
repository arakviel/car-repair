...
class RegionAddressValidator extends AddressValidator {

    protected RegionAddressValidator(Map<String, List<String>> validationMessages) {
    ...
    public boolean validate(Address address) {
        ...
        List<String> messages = InputValidator.getInstance().getErrorMessages(address.getRegion(), 2, 128, true);

        if (!messages.isEmpty()) {
            validationMessages.put("region", messages);
